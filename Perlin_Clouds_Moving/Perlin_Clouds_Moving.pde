...
float increment = 0.005; // Adjust this to change how dense clouds are (0.02 is pretty dense, 0.005 pretty sparse)
float zoff = 0;  // We'll move through 3D Perlin noise space using this 
...
void draw() {
  ...
  // Make clouds "move" if the "M" key is held down
  if (keyPressed) {
    if (key == 'm') {
      // To make the clouds "move" or change shape, we just move through Perlin noise space along the z-axis
      zoff += 0.02;
      println(zoff);
    }
  }
  ...
  for (int x = 0; x < width; x++) {
    ...
    for (int y = 0; y < height; y++) {
      ...
      float saturation = noise(xoff, yoff, zoff) * 100;
